...
AddEventHandler('QRCore:Client:OnPlayerLoaded', function()
    ...
    isHandcuffed = false
    TriggerServerEvent("QRCore:Server:SetMetaData", "ishandcuffed", false)
...
RegisterNetEvent('QRCore:Client:OnJobUpdate', function(JobInfo)
    if JobInfo.name == "sheriff" and PlayerJob.name ~= "sheriff" then
        CreatePrompts()
...
RegisterNetEvent('sheriff:client:UpdateBlips', function(players)
    if Config.ShowBlips then
        if PlayerJob and (PlayerJob.name == 'sheriff' or PlayerJob.name == 'ambulance') and onDuty then
            if DutyBlips then
                for k, v in pairs(DutyBlips) do
                    RemoveBlip(v)
                end
...
RegisterNetEvent('sheriff:client:sheriffAlert', function(coords, text)
    QRCore.Functions.Notify({text = text, caption = street1name.. ' ' ..street2name}, 'sheriff')
    local transG = 250
